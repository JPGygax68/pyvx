...
:Note: This call should only be used once within the lifetime of a context for a specific structure. \* :In group: group_adv_array  
    '''
...
def RegisterAutoAging(graph, delay):
    '''
Register a delay for auto-aging. \* This function registers a delay object to be auto-aged by the graph. This delay object will be automatically aged after each successful completion of this graph. Aging of a delay object cannot be called during graph execution.  A graph abandoned due to a node callback will trigger an auto-aging. \* If a delay is registered for auto-aging multiple times in a same graph, the delay will be only aged a single time at each graph completion. If a delay is registered for auto-aging in multiple graphs, this delay will aged automatically after each successful completion of any of these graphs. \*  :graph: [in] The graph to which the delay is registered for auto-aging.
 :delay: [in] The delay to automatically age. \* :Returns: A vx_status_e enumeration.
...
def CreateArray(context, item_type, capacity):
    '''
Creates a reference to an Array object. \* User must specify the Array capacity (i.e., the maximal number of items that the array can hold). \*  :context: [in] The reference to the overall Context.
  ...
  *VX_TYPE_RECTANGLE* for *vx_rectangle_t*.

  *VX_TYPE_KEYPOINT* for *vx_keypoint_t*.

  *VX_TYPE_COORDINATES2D* for *vx_coordinates2d_t*.

  *VX_TYPE_COORDINATES3D* for *vx_coordinates3d_t*.

  *vx_enum* returned from *vxRegisterUserStruct*.
 :capacity: [in] The maximal number of items that the array can hold. \* \returns An array reference *vx_array*. Any possible errors preventing a  successful creation should be checked using *vxGetStatus*. \* :In group: group_array  
    '''
...
\endcode \*  :graph: [in] The reference to the parent graph.
...
\returns A array reference *vx_array*. Any possible errors preventing a  successful creation should be checked using *vxGetStatus*. \* :In group: group_array  
    '''
...
def QueryArray(arr, attribute, ptr, size):
    '''
Queries the Array for some specific information. \*  :arr: [in] The reference to the Array.
 :attribute: [in] The attribute to query. Use a *vx_array_attribute_e*.
 :ptr: [out] The location at which to store the resulting value.
 :size: [in] The size in bytes of the container to which *ptr* points. \* :Returns: A *vx_status_e* enumeration.
  ...
  :VX_ERROR_INVALID_PARAMETERS: If any of the other parameters are incorrect. \* :In group: group_array  
    '''
    return lib.vxQueryArray(arr, attribute, ptr, size)
    
def AddArrayItems(arr, count, ptr, stride):
    '''
Adds items to the Array. \* This function increases the container size. \* By default, the function does not reallocate memory, so if the container is already full (number of elements is equal to capacity) or it doesn't have enough space, the function returns *VX_FAILURE* error code. \*  :arr: [in] The reference to the Array.
 :count: [in] The total number of elements to insert.
 :ptr: [in] The location from which to read the input values.
 :stride: [in] The number of bytes between the beginning of two consecutive elements. \* :Returns: A *vx_status_e* enumeration.
...
def TruncateArray(arr, new_num_items):
    '''
Truncates an Array (remove items from the end). \*  :arr: [in,out] The reference to the Array.
 :new_num_items: [in] The new number of items for the Array. \* :Returns: A *vx_status_e* enumeration.

  :VX_SUCCESS: No errors.

  :VX_ERROR_INVALID_REFERENCE: If the *arr* is not a *vx_array*.

  :VX_ERROR_INVALID_PARAMETERS: The *new_size* is greater than the current size. \* :In group: group_array  
    '''
...
def CreateObjectArray(context, exemplar, count):
    '''
Creates a reference to an ObjectArray of count objects. \* It uses the metadata of the exemplar to determine the object attributes,  ignoring the object data. It does not alter the exemplar or keep or release  the reference to the exemplar. For the definition of supported attributes see  *vxSetMetaFormatAttribute*. In case the exemplar is a virtual object  it must be of immutable metadata, thus it is not allowed to be dimensionless or formatless.  \*  :context: [in] The reference to the overall Context.
 :exemplar: [in] The exemplar object that defines the metadata of the created objects in the ObjectArray. 
 :count: [in] Number of Objects to create in the ObjectArray.   \* \returns An ObjectArray reference *vx_object_array*. Any possible errors preventing a  successful creation should be checked using *vxGetStatus*. Data objects are not initialized by this function. \* :In group: group_object_array  
    '''
    return lib.vxCreateObjectArray(context, exemplar, count)
    
def CreateVirtualObjectArray(graph, exemplar, count):
    '''
Creates an opaque reference to a virtual ObjectArray with no direct user access. \* This function creates an ObjectArray of count objects with similar behavior as  *vxCreateObjectArray*. The only difference is that the objects that are  created are virtual in the given graph. \*  :graph: [in] Reference to the graph where to create the virtual ObjectArray.  
...
def GetObjectArrayItem(arr, index):
    '''
Retrieves the reference to the OpenVX Object in location index of the ObjectArray.  \* This is a vx_reference, which can be used elsewhere in OpenVX. A call to vxRelease<Object> or *vxReleaseReference* is necessary to release the Object for each call to this function.  \*  :arr: [in] The ObjectArray.
...
def ReleaseObjectArray(arr):
    '''
Releases a reference of an ObjectArray object. \* The object may not be garbage collected until its total reference and its contained objects  count is zero. After returning from this function the reference is zeroed/cleared. \*  :arr: [in] The pointer to the ObjectArray to release.
:Returns: A *vx_status_e* enumeration.
...
def QueryObjectArray(arr, attribute, ptr, size):
    '''
Queries an atribute from the ObjectArray.  \*  :arr: [in] The reference to the ObjectArray.
...
:Returns: A *vx_status_e* enumeration.
  ...
  :VX_ERROR_INVALID_PARAMETERS: If any of the other parameters are incorrect. \* :In group: group_object_array  
    '''
...
:Note: For vx_image, a specific attribute can be used to specify the valid region evolution. This information is not a meta data. \*  :meta: [in] The reference to the vx_meta_format struct to set 
...
def SetMetaFormatFromReference(meta, exemplar):
    '''
Set a meta format object from an exemplar data object reference \* This function sets a vx_meta_format object from the meta data of the exemplar \*  :meta: [in] The meta format object to set
